# Callisto Cocktail Menu Training Review

Private review package for the Callisto Volume 2 cocktail-menu training materials.

## Review files

- `index.html`: training-package home
- `Callisto Cocktail Menu - Study Guide.html`: interactive study guide
- `Callisto Cocktail Menu - Employee Test.html`: 30-minute employee assessment
- `Callisto Cocktail Menu - Complete Reference.pdf`: complete 31-cocktail training reference
- `Callisto Volume 2 Edited Version 3 - Source Menu.pdf`: authoritative edited source menu, provided as a private release download because of its file size

Download the repository and open `index.html` in a browser to use the interactive files.

## Current assessment architecture

- 20 scored ingredient-recognition questions
- 14 unique options per ingredient question
- Exactly four selected components per ingredient question
- 10 one-point quick checks
- 3 unscored staff-perspective prompts
- 90 possible points on every generated attempt
- Correct component: +1 point
- Incorrect component: -1 point, with a zero-point floor per question
- Answers lock when the employee advances
- Fixed category distribution, including all three non-alcoholic cocktails
- Wall-clock 30-minute deadline that continues across reloads

## Accuracy audit

Before this review copy was published:

- All 31 cocktail records were compared with the authoritative source PDF.
- All 21 source pages were independently rendered and visually reviewed.
- The supplied source copy was verified byte-for-byte against the original attachment.
- All 10 quick-check answer keys were validated.
- Equivalent source spellings such as `Pinapple`/`Pineapple` and `Planteray 3 Star`/`3 Stars` are prevented from appearing as false distractors.
- 200 generated assessments were checked for question counts, fixed category distribution, 14 unique options, valid keys, false distractors, non-alcoholic coverage, fixed 90-point totals, answer locking, scoring, and 390-pixel mobile fit.

## Confidentiality

Internal Callisto training material. Do not make this repository public or redistribute the cocktail builds without authorization.
